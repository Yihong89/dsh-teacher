...
class SqliteQuestionStore {
  ...
  /**
   * Upsert a course by (workspace, title): an existing course with the same
   * title in the workspace is replaced (questions rewritten, id kept, updated
   * bump), otherwise a NEW course is inserted. Other courses in the workspace
   * are never touched — this is what allows multiple subject courses
   * (English, Science, Math…) to coexist per workspace.
   */
  upsertCourse({ workspace, title, source, questions }) {
    const now = Date.now()
    this.db.exec('BEGIN')
    try {
      const existing = this.db.prepare(
        'SELECT id FROM courses WHERE workspace = ? AND title = ? ORDER BY updated_at DESC LIMIT 1',
      ).get(workspace, title)
      let courseId
      if (existing !== undefined) {
        ...
        ).run(workspace, title, source ?? null, now, now)
  ...
  /** The most recent un-analyzed quiz run for a workspace, or null. */
  latestPendingRun(workspace) {
    const row = this.db.prepare(
      "SELECT id, workspace FROM quiz_runs WHERE status = 'pending' AND workspace = ? ORDER BY created_at DESC, id DESC LIMIT 1",
    ).get(workspace)
    return row === undefined ? null : { runId: Number(row.id), workspace: row.workspace }
  }

  /** The most recent un-analyzed run under a real directory workspace, or null. */
  latestPendingRunDirectory() {
    const row = this.db.prepare(
      "SELECT id, workspace FROM quiz_runs WHERE status = 'pending' AND (workspace LIKE '/%' OR workspace LIKE '~%') ORDER BY created_at DESC, id DESC LIMIT 1",
    ).get()
    return row === undefined ? null : { runId: Number(row.id), workspace: row.workspace }
  }

  /** All courses in a workspace, newest first, with question counts. */
  listCourses(workspace) {
    return this.db.prepare(`
      ...
      FROM courses c WHERE c.workspace = ? ORDER BY c.updated_at DESC
    `).all(workspace).map((row) => ({
  ...
  /** The workspace's selected course id (the one the user last chose), or null. */
  getSelectedCourseId(workspace) {
    const row = this.db.prepare(
      'SELECT selected_course_id FROM preferences WHERE workspace = ?',
    ).get(workspace)
    return row === undefined ? null : Number(row.selected_course_id)
  }

  /** Remember the workspace's selected course id. */
  setSelectedCourseId(workspace, courseId) {
    this.db.prepare(
      'INSERT INTO preferences (workspace, selected_course_id) VALUES (?, ?) ON CONFLICT(workspace) DO UPDATE SET selected_course_id = excluded.selected_course_id',
    ).run(workspace, Number(courseId))
  }

  latestCourse(workspace) {
    const course = this.db.prepare(
      'SELECT id, title, source FROM courses WHERE workspace = ? ORDER BY updated_at DESC LIMIT 1',
    ).get(workspace)
  ...
  /** Most recently updated course keyed by a real directory workspace, or null. */
  latestDirectoryCourse() {
    const course = this.db.prepare(
      "SELECT id, workspace, title, source FROM courses WHERE workspace LIKE '/%' OR workspace LIKE '~%' ORDER BY updated_at DESC LIMIT 1",
    ).get()
    if (course === undefined) return null
    return {
      ...this.courseWith(course),
  ...
  publicCourse(workspace) {
    const course = this.latestCourse(workspace)
...
class JsonQuestionStore {
  ...
  upsertCourse({ workspace, title, source, questions }) {
    ...
    let course = this.data.courses
      .filter((c) => c.workspace === workspace && c.title === title)
    ...
    if (course !== undefined) {
      ...
      this.data.courses.push({
        id: courseId, workspace, title, source: source ?? null, createdAt: now, updatedAt: now,
      })
  ...
  latestPendingRun(workspace) {
    const run = this.data.quizRuns
      .filter((r) => r.status === 'pending' && r.workspace === workspace)
  ...
  latestPendingRunDirectory() {
    const run = this.data.quizRuns
      .filter((r) => r.status === 'pending' && (r.workspace.startsWith('/') || r.workspace.startsWith('~')))
      .sort((a, b) => b.createdAt - a.createdAt)[0]
    return run === undefined ? null : { runId: run.id, workspace: run.workspace }
  }
  ...
  getSelectedCourseId(workspace) {
    const pref = this.data.preferences?.find((p) => p.workspace === workspace)
  ...
  setSelectedCourseId(workspace, courseId) {
    ...
    const pref = this.data.preferences.find((p) => p.workspace === workspace)
    if (pref === undefined) this.data.preferences.push({ workspace, selected_course_id: Number(courseId) })
  ...
  latestDirectoryCourse() {
    const course = this.data.courses
      .filter((c) => c.workspace.startsWith('/') || c.workspace.startsWith('~'))
      .sort((a, b) => b.updatedAt - a.updatedAt)[0]
    if (course === undefined) return null
    return {
      ...this.courseWith(course),
