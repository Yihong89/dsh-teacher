...
class SqliteQuestionStore {
  ...
  upsertCourse({ workspace, title, source, questions }) {
    ...
    try {
      this.db.prepare(
        'DELETE FROM questions WHERE course_id IN (SELECT id FROM courses WHERE workspace = ?)',
      ).run(workspace)
      this.db.prepare('DELETE FROM courses WHERE workspace = ?').run(workspace)
...
class JsonQuestionStore {
  ...
  upsertCourse({ workspace, title, source, questions }) {
    this.data.courses = this.data.courses.filter((c) => c.workspace !== workspace)
    const now = Date.now()
...
export class QuestionStore {
  ...
  static async open(filePath) {
    ...
    if (filePath.endsWith('.json')) {
      let data = { courses: [], questions: [], quizRuns: [] }
      try {
