 ...
 *  - `conversation.session.header.actions` "quiz" button (opens the LLM-free
 *    quiz popup fed by the `teacherQuiz` projection)
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var store = { open: false, gaps: [], quizOpen: false, quiz: null, listeners: [] }
    function emit() {
    ...
    // ---- LLM-free quiz popup --------------------------------------------
    var PANEL_WRAP = {
      position: 'fixed', right: 16, top: 72, zIndex: 1000, maxWidth: 520, width: '92vw',
    }
    var QUIZ_INPUT = {
    ...
    function QuizPanel() {
      ...
      var course = quiz !== null ? quiz.course : null
      var questions = course !== null && Array.isArray(course.questions) ? course.questions : []
      if (questions.length === 0) {
        return h('div', { style: PANEL_WRAP },
          h('div', { style: Object.assign({}, CARD, { boxShadow: '0 8px 30px rgba(0,0,0,.25)' }), className: 'dsh-teacher-quiz-panel' },
            h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 } },
    ...
    function QuizBody(props) {
      ...
      return h('div', { style: PANEL_WRAP },
        h('div', { style: Object.assign({}, CARD, { boxShadow: '0 8px 30px rgba(0,0,0,.25)', maxHeight: '80vh', overflow: 'auto' }), className: 'dsh-teacher-quiz-panel' },
          ...
          h('div', { style: { display: 'flex', justifyContent: 'space-between', marginTop: 10 } },
            ...
            }, index === questions.length - 1 ? 'Finish & analyze ✓' : 'Next →'),
          ),
          h('div', { style: { marginTop: 8, fontSize: 11, opacity: .55 } },
    ...
    exports.apply = function (ctx) {
      ...
      slots.inject('conversation.session.header.actions', function () {
        return slots.register(
          { name: 'conversation.session.header.actions', id: 'dsh-teacher-quiz', order: 14, label: function () { return 'Quiz' } },
          QuizButton,
        )
      })
