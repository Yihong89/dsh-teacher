...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- TTS speak toggle ------------------------------------------------
    // The teacher's voice is generated by Qwen3-TTS (VoiceDesign) on the host
    // (same service as dsh-sister) with a cool 御姐 (onee-san) voice. The
    // client fetches a WAV from /dsh-teacher/tts and plays it in an <audio>
    // queue — sound comes out of the user's own machine.
    var TEACHER_INSTRUCT = '清冷柔和的成年女声，御姐音色，语速平缓，语调沉稳优雅，带着淡淡的温柔与从容，听起来可靠又亲切。'
    var audioQueue = []
    ...
    /** Speak text with the Qwen3-TTS 御姐 voice via the host proxy. */
    function speakBrowser(text) {
      if (typeof window === 'undefined' || typeof window.fetch !== 'function') return
      if (!text || !String(text).trim()) return
      var clean = String(text).trim()
      fetch('/dsh-teacher/tts?text=' + encodeURIComponent(clean) + '&instruct=' + encodeURIComponent(TEACHER_INSTRUCT), {
        ...
        .catch(function () {
          // TTS service unreachable — silent.
        })
    }
    ...
    function SpeakToggle(props) {
      ...
      // sessions (or reloading) never re-reads an old message as "new". The
      // cursor is the highest assistant seq already read; a real new message
      // (larger seq) still speaks.
      var spokenSeqCursor = React.useRef(null) // { sessionId, seq }
      var spokenMsgRef = React.useRef(null)
      var pendingSpeakRef = React.useRef(null)
      var SPOKEN_CURSOR_KEY = 'dsh-teacher.spoken-cursor'
    ...
    // name/inject/apply).
    exports._test = { assistantNodeText, latestAssistantText, unwrapNode, SPEAK_CURSOR_KEY: 'dsh-teacher.spoken-cursor' }
