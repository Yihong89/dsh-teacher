...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function latestAssistantText(session) {
      if (session === null || session === undefined || !session.chat) return null
      ...
      for (var i = order.length - 1; i >= 0; i--) {
        ...
        var blocks = n.blocks || []
        var text = blocks
          .filter(function (b) { return b !== null && b.type === 'text' && typeof b.text === 'string' })
    ...
    function SpeakToggle(props) {
      ...
      var session = (props.useSession ? props.useSession(function (s) { return s }) : null)
      ...
      // just next_question). Dedupes by message seq, and waits a beat so the
      // learner sees the text before the sound starts.
      ...
      React.useEffect(function () {
        ...
        if (msg === null) return
        var prev = spokenMsgRef.current
        if (prev !== null && prev.seq === msg.seq && prev.text === msg.text) return
        spokenMsgRef.current = msg
        var timer = setTimeout(function () { speakBrowser(msg.text) }, 1000)
