...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- voice engine (shared dsh-voice-core) ------------------------------
    // The teacher's voice is generated by Qwen3-TTS via the shared
    // dsh-voice-core engine (proxy /dsh-teacher/tts), with a cool 御姐
    // (onee-san) voice. All TTS/audio/auto-read UI lives in the core; the
    // teacher client keeps only its own tool cards + quiz/gaps business.
    ...
    var teacherVoice = core.createVoiceClient({
      presetName: 'teacher',
      ttsPath: '/dsh-teacher/tts',
      styles: TEACHER_STYLES,
      defaultStyle: 'onee',
      previewText: '你好呀，我是你的老师～这节课我们一起把知识弄明白，好吗？',
      cheerTitle: '💛 Teacher says…',
    })

    // Re-export core helpers for tests.
    var _test = core._test
    _test.SPEAK_CURSOR_KEY = 'dsh-voice.spoken-cursor.teacher'
    ...
    exports.apply = function (ctx) {
      ...
      pluginCtx = ctx
      // Voice engine: speak toggle + style picker + cheer chip from the core.
      teacherVoice.apply(ctx)
