...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function speakBrowser(text) {
      try { console.log('[tts-debug] speakBrowser called:', String(text).slice(0, 50)) } catch (e) {}
    ...
    function SpeakToggle(props) {
      ...
      React.useEffect(function () {
        ...
        try { console.log('[tts-debug] effect fired, msg:', msg === null ? 'null' : JSON.stringify({ seq: msg.seq, text: msg.text.slice(0, 40) }), 'isTeacher:', isTeacher, 'enabled:', value.speakEnabled) } catch (e) {}
        if (msg === null) return
