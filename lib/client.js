...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SpeakToggle(props) {
      ...
      // Browser-TTS listener: when the teacher records a speak request
      // (teacher/spoken → lastSpoken), play it with the browser's built-in
      // speech synthesis — the sound comes out of the user's own machine.
      ...
      React.useEffect(function () {
        ...
        try {
          ...
          var utter = new window.SpeechSynthesisUtterance(String(last.text))
          var isChinese = /[\\u4e00-\\u9fff]/.test(String(last.text))
          var want = isChinese ? 'zh' : 'en'
          var voice = null
          try {
            var voices = window.speechSynthesis.getVoices()
            voice = voices.find(function (v) { return v.lang && v.lang.toLowerCase().indexOf(want) === 0 }) || null
          } catch (e) {}
