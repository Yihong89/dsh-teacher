...
// Shared voice engine: Qwen3-TTS proxy, speak/cheer tools, voiceSpeak
// projection, /speak /cheer /voice commands.
import { applyVoice, foldVoiceState, SPOKEN_EVENT as VOICE_SPOKEN_EVENT } from 'dsh-voice-core'
...
export async function apply(ctx) {
  ...
  // Shared voice engine: Qwen3-TTS 御姐 proxy (/dsh-teacher/tts), speak/cheer
  // tools, /speak /cheer /voice commands, and the voiceSpeak projection.
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      ...
      // Socratic walk: ALWAYS read the question aloud (Qwen3-TTS 御姐音 on the
      // user's machine) unless TTS is muted — no model compliance needed.
      if (foldVoiceState(agent.session.events).speakEnabled) {
        try {
          agent.session.append(VOICE_SPOKEN_EVENT, { text: q.prompt, voice: null })
        } catch (error) {
          controller.ctx.logger?.warn?.(`dsh-teacher: failed to append voice/spoken: ${error}`)
        }
      }
      return {
        id: q.id,
  ...
  // The speak tool is provided by the shared dsh-voice-core engine
  // (applyVoice above) — 御姐 Qwen3-TTS voice, not teacher-mode-gated.
}
