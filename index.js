...
export async function apply(ctx) {
  ...
  if (webServer !== undefined && typeof webServer.register === 'function') {
    // Qwen3-TTS proxy: the teacher's voice is generated by the same local
    // Qwen3-TTS service as dsh-sister (127.0.0.1:3091). The browser can't
    // reach that loopback port, so proxy /dsh-teacher/tts here.
    const TTS_BASE = process.env.DSH_SISTER_TTS_URL || 'http://127.0.0.1:3091'
    webServer.register({
      kind: 'exact',
      path: '/dsh-teacher/tts',
